...
- WebSocket 转发：实时转发消息给 WebSocket 客户端
...
- WS_CACHE_DURATION：消息缓存时间（毫秒），默认 120000（2 分钟）
- WS_HEARTBEAT_INTERVAL：心跳检测间隔（毫秒），默认 30000（30 秒）
- WS_HEARTBEAT_TIMEOUT：心跳超时时间（毫秒），默认 90000（90 秒）
...
WebSocket 服务可以实时转发企业微信和 Telegram 收到的消息给所有连接的客户端。

### 连接方式

连接 `ws://host:port/ws`，支持以下查询参数：
...
示例：
```
ws://127.0.0.1:3123/ws?user=carrot.autolock&os=windows&arch=x64&desc=Microsoft%20Windows%2010.0.19045
```
...
- `connected`：连接成功，包含 `clientId`、`heartbeatInterval`、`heartbeatTimeout`
- `message`：转发消息，包含 `data` 字段
- `pong`：心跳响应

### 客户端 ID 生成规则

- 如果提供了 `user`、`os`、`arch` 参数：`{user}@{os}-{arch}`
  - 例如：`carrot.autolock@windows-x64`
- 如果参数不完整：`client_{timestamp}_{random}`
...
1. 客户端发送：`{"type":"ping"}`
2. 服务端回复：`{"type":"pong","timestamp":"..."}`
3. 建议心跳间隔：小于 60 秒（默认超时时间）
4. 超时未收到心跳，服务端将主动断开连接

### 特性

- 新客户端连接时自动发送最近一条消息（在缓存时间内）
- 支持心跳检测和超时断开
- 可配置缓存时间和心跳参数
