...
async function sendTelegramMessage(content: string, type: ContentType): Promise<void> {
  ...
  if (TELEGRAM_PROXY_TYPE && TELEGRAM_PROXY_HOST && TELEGRAM_PROXY_PORT) {
    const proxyUrl = TELEGRAM_PROXY_TYPE === 'socks'
    ...
    proxyAgent = TELEGRAM_PROXY_TYPE === 'socks'
      ? new SocksProxyAgent(proxyUrl)
      : new HttpsProxyAgent(proxyUrl);
    
    logger.debug({ type: TELEGRAM_PROXY_TYPE, host: TELEGRAM_PROXY_HOST, port: TELEGRAM_PROXY_PORT }, 'Telegram using proxy');
  }
...
export async function registerPushApiRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<{ Body: PushRequest }>('/send', async (request, reply) => {
    ...
    if (!['wecom', 'telegram'].includes(platform)) {
      ...
      return reply.code(400).send({ success: false, error: 'Invalid platform. Use "wecom" or "telegram"' });
    }

    // 验证内容类型
    if (!['text', 'markdown'].includes(type)) {
      logger.warn({ ip: request.ip, type }, 'Invalid content type');
      return reply.code(400).send({ success: false, error: 'Invalid type. Use "text" or "markdown"' });
    }
    ...
    try {
      ...
      logger.info({ platform, type, contentLength: content.length }, 'Push message sent successfully');
