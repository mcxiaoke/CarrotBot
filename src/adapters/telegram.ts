...
export class TelegramAdapter implements IAdapter {
    ...
    constructor(config: TelegramConfig) {
        ...
        if (config.proxy) {
            ...
            const proxyUrl =
                type === 'socks' ? `socks5://${host}:${port}` : `http://${host}:${port}`
            ...
            // 创建代理 Agent 用于文件下载
            this.proxyAgent =
                type === 'socks' ? new SocksProxyAgent(proxyUrl) : new HttpsProxyAgent(proxyUrl)
    ...
    parseMessage(raw: unknown): StandardMessage {
        ...
        if (msg.text) {
            ...
            logger.debug(
                { fileId: mediaUrl, photoSize: largestPhoto?.file_size },
                'Parsed Telegram photo'
            )
        } else if (msg.voice) {
            // 语音消息
            content = '[语音]'
            ...
            logger.debug(
                { fileId: mediaUrl, fileName: msg.document.file_name },
                'Parsed Telegram document'
            )
        } else if (msg.sticker) {
            // 贴纸消息
            content = '[贴纸]'
    ...
    async downloadFile(fileId: string): Promise<{ buffer: Buffer; filename?: string }> {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                logger.debug(
                    { fileId, fileUrl, fileSize: file.file_size, attempt },
                ...
                logger.debug(
                    { fileId, filename, size: buffer.length, attempt },
                ...
                logger.warn(
                    { fileId, attempt, maxRetries: MAX_RETRIES, err: lastError.message },
        ...
        logger.error(
            { err: lastError, fileId },
            'Failed to download Telegram file after all retries'
        )
        throw lastError
    }
    ...
    connect(): void {
        ...
        this.bot.on('message', async (msg) => {
            ...
            // 保存收到的消息到数据库
            try {
                await saveIncomingMessage(standardMsg, (fileId: string) =>
                    this.downloadFile(fileId)
                )
                logger.info(
                    {
                        msgtype: standardMsg.msgType,
                        from: standardMsg.from,
                        hasMedia: !!standardMsg.mediaUrl
                    },
                    'Telegram message saved'
                )
            } catch (err) {
                logger.error(
                    { err, msgtype: standardMsg.msgType, from: standardMsg.from },
